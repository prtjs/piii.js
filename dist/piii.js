...
var filtersToRegex = __webpack_require__(6);
var isArray = __webpack_require__(2);
var removeAccents = __webpack_require__(22).remove;

var Piii = function () {
  function Piii() {
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    _classCallCheck(this, Piii);
    ...
    var _options$aliases = options.aliases,
        ...
        _options$repeated = options.repeated,
        repeated = _options$repeated === undefined ? true : _options$repeated,
        _options$censor = options.censor,
        censor = _options$censor === undefined ? function () {
      return "*";
    } : _options$censor,
        _options$cleaner = options.cleaner,
        cleaner = _options$cleaner === undefined ? removeAccents : _options$cleaner;


    if (!isArray(filters)) throw TypeError("must be an array");

    this._filters = filtersToRegex(filters, repeated, aliases);
    this._censor = typeof censor === "function" ? censor : function () {
      return censor;
    };
  ...
  _createClass(Piii, [{
    key: "filter",
    value: function filter(string) {
      return _filter(string, this._filters, this._censor, this._cleaner);
    }
  }, {
    key: "has",
...
function filter(string, regex, censor, clear) {
  ...
  var match = clear(string).match(regex);

  if (match === null) return string;

  var badWords = RegExp("\\b(" + match.join("|") + ")\\b");

  return clear(string).split(badWords).map(function (val, index, array) {
    if (badWords.test(val)) return censor(val);

    var start = array.slice(0, index).join("").length;
    var end = val.length;
...
var arrayToRegex = __webpack_require__(7);
var buildAliases = __webpack_require__(14);
var addAliases = __webpack_require__(20);
var objectIsEmpty = __webpack_require__(21);
...
function filtersToRegex(filters, repeated, aliases) {
  ...
  regex = filters.map(arrayToRegex).join("|");

  regex = repeated ? regex.replace(/(\w)/g, "$1+") : regex;
...
var justJoin = __webpack_require__(8);
var joinArrays = __webpack_require__(9);
var joinStrings = __webpack_require__(10);
var reverse = __webpack_require__(1);
var isArray = __webpack_require__(2);
var isString = __webpack_require__(0);
var hasOnlyLetters = __webpack_require__(13);
...
var reverse = __webpack_require__(1);
var removeRepeatedValues = __webpack_require__(3);
...
var reverse = __webpack_require__(1);
var removeRepeatedValues = __webpack_require__(3);
var toLowerCase = __webpack_require__(11);
...
var isString = __webpack_require__(0);
var hasRepeatedValues = __webpack_require__(15);
var createRegex = __webpack_require__(16);
var isJustOneLetter = __webpack_require__(18);
var isValid = __webpack_require__(19);

function buildAliases(aliases) {
  var keys = Object.keys(aliases);
  var values = Object.values(aliases);

  if (!keys.every(isJustOneLetter) || !values.every(isValid)) throw new Error("invalid alias");

  return keys.map(function (key) {
    if (aliases[key].includes(key)) throw new Error("invalid alias");

    if (hasRepeatedValues(aliases[key])) throw new Error("can't have repeated values");
...
function createRegex(original, aliases) {
  return "[" + escapeStringRegexp([original].concat(_toConsumableArray(aliases)).join("")) + "]";
}
...
var isArray = __webpack_require__(2);
var isString = __webpack_require__(0);
var isJustOneChar = function isJustOneChar(char) {
  return char.length === 1;
};

function isValid(array) {
  return isArray(array) && array.every(isString) && array.every(isJustOneChar);
}
...
function addAliases(string, aliases) {
  var stringArray = Array.from(string).map(function (char) {
    var aliasFound = aliases.find(function (alias) {
      return alias.char === char;
    });

    return aliasFound ? aliasFound.value : char;
  });
...
function isEmpty(value) {
  return value === null || value === undefined || typeof value === "boolean" || Object.keys(value).length === 0;
}
