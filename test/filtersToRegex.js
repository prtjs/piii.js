...
describe("filtersToRegex", () => {
  ...
  it("deve criar um regex sem repetição e sem aliases", () => {
    expect(filtersToRegex(filters, false, {})).toEqual(/\b((foo|bar))\b/g);
  });
  it("deve criar um regex com repetição", () => {
    expect(filtersToRegex(filters, true, {})).toEqual(/\b((f+o+o+|b+a+r+))\b/g);
  });
  it("deve criar um regex sem repetição e com aliases", () => {
    expect(filtersToRegex(filters, false, {a: ["4"]})).toEqual(/\b((foo|b[a4]r))\b/g);
  });
  it("deve criar um regex com repetição e com aliases", () => {
    expect(filtersToRegex(filters, true, {a: ["4"]})).toEqual(/\b((f+o+o+|b+[a4]+r+))\b/g);
  });
});
